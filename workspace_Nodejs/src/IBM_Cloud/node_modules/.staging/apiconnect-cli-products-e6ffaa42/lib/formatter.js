  ...
  _.keys(products).forEach(function(key) {
    ...
    if (prod.spaceName) {
      logger.writeln(g.f('%s:%s in %s:%s:%s',
        prod.document.info.name, prod.document.info.version, orgName, shortName, prod.spaceName));
    } else {
      logger.writeln(g.f('%s:%s in %s:%s',
        prod.document.info.name, prod.document.info.version, orgName, shortName));
    }
  });
};
