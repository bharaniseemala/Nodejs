...
var AVAILABLE_LB_VERSIONS = {
  '2.x': { description: g.f('long term support') },
  '3.x': { description: g.f('current') },
};

function describeLBVersion() {
  var versionNames = Object.keys(AVAILABLE_LB_VERSIONS);
  return versionNames.map(function(version) {
    return {
      name: version + ' (' + AVAILABLE_LB_VERSIONS[version].description + ')',
      value: version,
    };
  });
};
...
function promptNewApp(options) {
  ...
  var availableLBVersions = describeLBVersion();
  var inputs = [ {
    type: 'input',
    ...
    default: availableLBVersions[0].value,
  }, {
    type: 'list',
    ...
    choices: function(answers) {
      ...
      .then(function(templates) {
        return templateFilter(templates, answers.loopbackVersion);
      })
