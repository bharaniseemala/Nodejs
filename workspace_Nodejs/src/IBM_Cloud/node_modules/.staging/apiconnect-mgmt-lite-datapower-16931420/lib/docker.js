...
const normalize = proj => (proj || '').toLowerCase().replace(/[\W_]/g, '');

const ports = exports.ports = project => {
  let _project = normalize(project);
  let serviceNames;
  return loadYaml(Path.resolve(packageRoot, dockerComposeYml))
    .then(dcyaml => {
      serviceNames = _.keys(dcyaml.services);
      return listByName(_.map(serviceNames, sn => `${_project}_${sn}`));
    })
    .then(containers => {
...
const logs = exports.logs = (project, services, options) => {
  ...
  if (typeof services === 'string')
    services = [services];
  return listByName(_.map(services || [''], s => `${_project}_${s}`))
    .then(containers => {
...
const details = exports.details = project => {
  let _project = normalize(project);
  let promises = [
    listByName(_project).then(c => ({ containers: c })),
    ports(_project)
  ];
