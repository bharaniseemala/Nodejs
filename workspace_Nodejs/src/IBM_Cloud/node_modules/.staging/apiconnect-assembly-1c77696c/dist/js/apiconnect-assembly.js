...
function ErrorFilterController($scope) {
  ...
  $scope.$watch('selectedNode.$$stopOnError', function() {
    if ($scope.selectedNode.$$stopOnError) {
      if (!$scope.selectedNode['stop-on-error']) $scope.selectedNode['stop-on-error'] = [];
    } else {
...
// seem to be unused...
  ...
  self.addParametersForOperation = function(selectedOperation) {
    ...
    parameters.forEach(function(parameter) {
      ...
      var variable = (parameter.in === "body") ? "request.body" : "request.parameters." + parameterName;
  ...
  self.showTargetDetails = function(target) {
    ...
    $mdDialog.show({
      ...
      if (response.remove === true || (_.isEmpty(response.from) && _.isEmpty(response.value) && _.isEmpty(response.default))) {
      ...
      if (response.default) {
        mapping.default = response.default;
      } else {
  ...
  function constructInputNodes(cssRule, cssClass, connectorNodeData, clickZoneData) {
    ...
    for (i = 0; i < mappableInputs.length; i++) {
      ...
      height = input.height();
      $scope.paths[input.attr("path")] = new Path(input.attr("path"), input.attr("dimensions"), input.attr("container"));
  ...
  function constructOutputNodes(cssRule, cssClass, connectorNodeData, clickZoneData) {
    ...
    for (i = 0; i < mappableTargets.length; i++) {
      ...
      $scope.paths[target.attr("path")] = new Path(target.attr("path"), target.attr("dimensions"), target.attr("container"));
      className = cssClass;
  ...
  self.constructCanvas = function(forceClear) {
    ...
    $(".mapper-inputs-container").on("scroll", scrollLeft);
    $(".mapper-outputs-container").on("scroll", scrollRight);
...
function targetConnectorController($scope, $mdDialog, mapping, target, paths) {
...
angular.module('apiconnect-assembly').controller('TargetConnectorController', ['$scope', '$mdDialog', 'mapping', 'target', 'paths', targetConnectorController]);
