    ...
    if (this.definitions) {
        ...
        try {
            ...
            if (schemaWithoutTargetNamespace) {
                console.log('A schema without a targetNamespace is not supported.  An error occurred while processing the schema:' + e);
            } else {
...
*/
function fromXML(obj, content, context) {
    try {
        ...
        throw new Error('Error occurred while parsing "' + escape(insertContext) + '"\n' + cleanupError(e) + '\n The content starts with [' + escape(insertContent) + ']');
    }
}
...
function preParse(fileContent) {
    ...
    if (wsdlDef && wsdlDef.length > 0) {
        ...
        // Remove wsdl:documentation under defintions, but only if there is NO wsdl:service
    ...
    // If the doc is not updated, return the original fileContent (we do this for performance reasons)
    if (!docUpdated) {
        return fileContent;
    } else {
...
*/
function getFileContent(location, fullPath, auth, context) {
    var def = q.defer();
            ...
            response.on('end', function() {
                if (response.statusCode >= 200 && response.statusCode < 300) {
                    ...
                    if (left == 'PK\u0003\u0004') {
                        ...
                        encoding = determineEncoding(content, location);
                    }
                    def.resolve(decode(content, encoding));
                } else {
                    // Failed to get a remote file
                    def.reject(fileContentError(context, 'Failed to retrieve the remote file from location: ' + location + '. Ensure the remote file is available. ' +
                            'The HTTP Response code is:' + response.statusCode));
                }
            });
        }).on('error', function(e) {
            def.reject(fileContentError(context, 'Failed to retrieve the remote file from location: ' + location + '. Ensure the remote file is available. ' + e));
        });
    } else if (location.substr(0, 6) == 'ftp://') {
        // fetch from FTP
        def.reject(fileContentError(context, 'Failed to retrieve the remote file from location: ' + location + '. FTP is not supported. '));
                ...
                if (location.length > 1000) {
                    ...
                    try {
                        if (!fs.existsSync(fullPath)) {
                            def.reject(fileContentError(context, 'The local file "' + resolvedPath + '" does not exist.'));
                        } else {
                            def.reject(fileContentError(context, 'The local file "' + resolvedPath + '" is not accessible: ' + err));
                        }
                    } catch (e) {
                        def.reject(fileContentError(context, 'The local file "' + resolvedPath + '" is not accessible: ' + e));
                    }
                }
            } else {
                var encoding = 'utf8'; // start with a common default encoding
...
function validateWSDLJson(wsdlJson, filename) {
    ...
    if (wsdlJson && wsdlJson.definitions) {
        ...
        ret.valid = false;
        ret.reason = 'WSDL document does not contain any valid content. File: ' + filename;
    }
    return ret;
}
...
*/
function contentToXMLorWSDL(file) {
    // protectDocumentation does a text replace (it does not load the DOM)
    file.content = protectDocumentation(file.content);

    // Convert to XML/WSDL, we do this before the 'preParse' Because
    // the soap module will catch common xsd and wsdl errors.
    try {
        file.doc = new soap.WSDL('', file.filename, {});
        if (file.doc.internalError) {
            throw file.doc.internalmyError;
        }
        fromXML(file.doc, file.content, file.filename);
        ...
        if (e.toString().indexOf('Unmatched element nesting') < 0) {
            throw e;
        }
    }

    // Now do a preParse (DOM load) of the original content and adjust it.
    // This DOM Load does not find common errors.
    file.content = preParse(file.content);
...
function toBuffer(obj) {
    ...
    try {
        ...
        try {
            ...
            throw new Error('Error trying to convert ' + obj.constructor.name + 'to Buffer: ' + error);
        }
    }
}
...
function determineEncoding(content, filename) {
    var ret = 'utf8';
    if (Buffer.isBuffer(content)) {
        ...
        if (ret == 'ucs4' || ret == 'ebcdic') {
            throw new Error('Unsupported encoding: ' + ret + ' in file: ' + filename);
        }
        ...
        if (content.length > bomLength) {
            ...
            try {
                ...
                var errMsg = e.message + ' in file: ' + filename;
...
function cleanupError(e) {
    ...
    if (e instanceof assert.AssertionError) {
        ...
        msg = 'Expected wsdl or xsd content.';
        ret = new Error(msg);
    }
    return ret;
}
