...
function generateSOAP(serviceName, wsdlId, wsdlJson, namespaces, serviceChildren, dict, refMap, options) {
    ...
    for (let j = 0; j < defMessages.length; j++) {
        ...
        let msgNsName = dictionary.resolveNameInNamespace(message['undefined'].name, message.xmlns, namespaces, message.tns ? message.tns : tns);
    ...
    for (let k = 0; k < defServices.length; k++) {
        ...
        if (service['undefined'].name == serviceName) {
            if (service.documentation) {
                swagger.info.description = u.cleanupDocumentation(service.documentation);
            }
            ...
            let portName = generatePort(swagger, serviceName, ports, namespaces, serviceChildren, dict);
            let isSoap12 = isSoap12Port(portName, serviceName, namespaces, serviceChildren);
            ...
            let localNamespaces = (portType && portType.xmlns) ? portType.xmlns : namespaces;
            ...
            for (let m = 0; m < portTypeOperations.length; m++) {
                ...
                if ((inputBody && inputBody['undefined'].use === 'encoded') ||
                  ...
                  (faultBody && faultBody['undefined'].use === 'encoded')) {
                    dictionary.annotateWithParserMessage(dict.dictEntry[dictionary.RESERVED_DEF].schema,
                       'The use=encoded setting is ignored on operation ' + op.operation['undefined'].name);
                }
                ...
                if (op.operation.input) {
                    ...
                    inputMessage = dictionary.resolveNameInNamespace(inputMessage, localNamespaces, namespaces, tns);
                    if (inputPartList) {
                    ...
                    if (isDocumentStyle) {
                        ...
                        let opMessageInput = getMessagePartType(inputMessage, messages, inputPartList, namespaces, inputContentParts, dict);
                    ...
                    msgNameLookup[inputMessage] = inputMessageName[0].type;
                }
                ...
                if (op.operation.output) {
                    ...
                    // must resolve the namespace here as this will normalise use of duplicate namespaces and prefixes
                    outputMessage = dictionary.resolveNameInNamespace(outputMessage, localNamespaces, namespaces, tns);
                    if (outputPartList) {
                    ...
                    if (isDocumentStyle) {
                        ...
                        let opMessageOutput = getMessagePartType(outputMessage, messages, outputPartList, namespaces, outputContentParts, dict);
                    ...
                    msgNameLookup[outputMessage] = outputMessageName[0].type;
                }
                ...
                if (op.operation.fault) {
                    ...
                    for (let n = 0; n < faultLen; n++) {
                        ...
                        // must resolve the namespace here as this will normalise use of duplicate namespaces and prefixes
                        faultMessage = dictionary.resolveNameInNamespace(faultMessage, localNamespaces, namespaces, tns);
                        ...
                        if (isDocumentStyle) {
                            ...
                            let opMessageFault = getMessagePartType(faultMessage, messages, faultPartList, namespaces, faultContentParts, dict);
                ...
                let operNamespace = inputBody ? inputBody['undefined'].namespace : null;
                let operName = inputMessage ? msgNameOperLookup[inputMessage] : op.name;
                let opDescription = op.operation.documentation ? u.cleanupDocumentation(op.operation.documentation) : '';
                ...
                if (inputMessage) {
                    ...
                    dict.exampleNeeded[inputNSName] = inMsgPropName ? inMsgPropName : op.name;

                    // A definition is created for the input header.
                    // Security is always added (but it is not required)
                    ...
                    if (inputHeaders) {
                        ...
                        for (let x = 0; x < headers.length; x++) {
                            ...
                            if (header['undefined'].use === 'encoded') {
                                dictionary.annotateWithParserMessage(dict.dictEntry[dictionary.RESERVED_DEF].schema, 'The use=encoded setting is ignored on operation ' + op.operation['undefined'].name);
                            }
                            let headerMessageName = dictionary.resolveNameInNamespace(header['undefined'].message, localNamespaces, namespaces);
                            ...
                            if (headerMessage) {
                                ...
                                for (let z = 0; z < headerParts.length; z++) {
                                    ...
                                    if (hdrPartName == headerPart) {
                                        ...
                                        if (hdrElemName) {
                                            let hdrElemNsName = dictionary.resolveNameInNamespace(hdrPart['undefined'].element, headerMessage.xmlns, namespaces);
                ...
                if (outputMessage) {
                    ...
                    dict.exampleNeeded[outputNSName] = outMsgPropName ? outMsgPropName : true;
                    ...
                    if (!outputHeaders) {
                        ...
                        for (let x = 0; x < headersOut.length; x++) {
                            ...
                            let headerOutMessageName = dictionary.resolveNameInNamespace(headerOut['undefined'].message, localNamespaces, namespaces);
                            ...
                            if (headerOutMessage) {
                                ...
                                for (let z = 0; z < headerOutParts.length; z++) {
                                    ...
                                    if (hdrOutPartName == headerOutPart) {
                                        ...
                                        if (hdrOutElemName) {
                                            let hdrOutElemNsName = dictionary.resolveNameInNamespace(hdrOutPart['undefined'].element, headerOutMessage.xmlns, namespaces);
                ...
                if (faultMessages.length > 0  || options.generateFaultDefs) {
                    ...
                    let faultHeaders = getFaultHeaders(bindingOper.operation.input, bindingOper.operation.output);
                    if (!faultHeaders) {
                        ...
                        for (let x = 0; x < headersFault.length; x++) {
                            ...
                            let headerFaultMessageName = dictionary.resolveNameInNamespace(headerFault['undefined'].message, localNamespaces, namespaces);
                            ...
                            if (headerFaultMessage) {
                                ...
                                for (let z = 0; z < hpartFaultLen; z++) {
                                    ...
                                    if (hdrFaultPartName == headerFaultPart) {
                                        ...
                                        if (hdrFaultElemName) {
                                            let hdrFaultElemNsName = dictionary.resolveNameInNamespace(hdrFaultPart['undefined'].element, headerFaultMessage.xmlns, namespaces);
            ...
            for (let messageName in usedMessages) {
                ...
                let prefix = u.getPrefixForNamespace(tns, namespaces);
                let msgDefinition = {
                    xml: {
                        namespace: tns,
                        prefix: prefix
                ...
                let partList = usedMessages[messageName];
                if (partList === true) {
                ...
                for (let i = 0; i < parts.length; i++) {
                    ...
                    let partNSName = dictionary.resolveNameInNamespace(partType, msg.xmlns ? msg.xmlns : namespaces, namespaces);
                    ...
                    if (partNameOK) {
                        ...
                        if (dictEntry) {
                            // found a custom type
                            let referencingContext = dict.elementNS[partNSName];
                            ...
                            let xso = genDefs.generateSwaggerXSO(dictEntry, dict, refMap, namespaces, referencingContext);
                            u.extendObject(property, xso);
                        } else {
                            let xso = genDefs.mapXSDTypeToSwagger(u.stripNamespace(partType));
                            u.extendObject(property, xso);
                        }
                        if (isDocumentStyle) {
                ...
                if (msgNameOperLookup[messageName]) {
                    dict.exampleNeeded[msgNSName] = msgNameOperLookup[messageName];
                } else {
                    dict.exampleNeeded[msgNSName] = true;
                }
            } // end for
        }
    }
...
*/
function generatePort(swagger, serviceName, ports, namespaces, serviceChildren, dict) {
    ...
    if (ports.length > 0) {
        ...
        if (ports.length > 1) {
            dictionary.annotateWithParserMessage(dict.dictEntry[dictionary.RESERVED_DEF].schema,
              'The service has multiple ports.  The service is generated using information in the first port.');
        }
    }
    return portName;
}
...
*/
function isSoap12Port(portName, serviceName, namespaces, serviceChildren) {
    var ret = false;
    var prefix = getSoap12Prefix(namespaces);
    if (prefix) {
        if (serviceChildren) {
            var services = serviceChildren;
...
*/
function getFaultHeaders(input, output) {
    var faultHeaders = [];
    ...
    if (inoutfault &&
        ...
        for (var p = 0; p < inoutfault.multipartRelated.part.length; p++) {
            ...
            if (part.content &&
                ...
                dictionary.annotateWithParserMessage(dict.dictEntry[dictionary.RESERVED_DEF].schema,
                  'Ignoring mime:content part:' + part.content['undefined'].part);
            }
        }
    }
    return ret;
...
function getMessagePartType(messageNsName, messages, partList, namespaces, contentPartList, dict) {
    contentPartList = contentPartList ? contentPartList : [];
    var ret = [];
    var opMessage = messages[messageNsName];
    if (opMessage) {
        var opMsgParts = u.makeSureItsAnArray(opMessage.part);
        ...
        for (var i = 0; i < len; i++) {
            ...
            if (contentPartList.indexOf(opPartName) < 0) {
                ...
                if (partNameOK) {
                    ...
                    // style=use.  In such cases, I believe
                    // the parts must be elements (not type).
                    var opPartType = opMsgPart['undefined'].type;
                    if (opPartType) {
                        dictionary.annotateWithParserMessage(dict.dictEntry[dictionary.RESERVED_DEF].schema,
                          'The wsdl part ' + opPartName + ' is defined with a type even though the style=document. An element was expected. This is tolerated but could cause problems.');
                    }
                    var elemName = '';
                    if (!opPartType) {
                    ...
                    if (opMessage.xmlns) {
                        opPartType = dictionary.resolveNameInNamespace(opPartType, opMessage.xmlns, namespaces);
                    } else {
                        opPartType = dictionary.getNsName(opPartType);
                    }
                    ret.push({
                        name: opPartName,
                        elemName: elemName,
