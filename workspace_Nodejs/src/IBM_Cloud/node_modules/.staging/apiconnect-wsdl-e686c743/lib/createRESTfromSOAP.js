...
/**
* Migrate a SOAP-Proxy or SOAP-REST from v6 to v5
* @param swaggerV6
* @param options (currently not used)
* @return swaggerv5
*/
...
*/
function getSwaggerForSOAPREST(soapSwagger, options) {
    options = options || {};
    if (typeof options.badgerfishDefs === 'undefined') {
        options.badgerfishDefs = true;
    }

    let service = soapSwagger['x-ibm-configuration']['wsdl-definition'].service;
    options.type = options.type || 'wsdl-to-rest';

    // Initialize the rest swagger
    let restSwagger = initRESTSwagger(service, options.type, soapSwagger['x-ibm-configuration']['wsdl-definition']);
    ...
    addAssemblySwitch(restSwagger, service, options);

    // Create a description of each operation
    let operationDesc = createOperationDesc(soapSwagger);

    // Add the definitions for th operation request, response, fault
    addRESTOperationDefinitions(operationDesc, restSwagger, service, options);
    ...
    addRESTDefinitions(soapSwagger, restSwagger, options);

    // Add the wiring of the request and response mappings
    addAssemblyWiring(restSwagger, operationDesc);

    // Add the SOAPFault _catch
    if (options.mapSOAPFaults) {
        addAssemblyCatchSOAPError(restSwagger, service, operationDesc, options);
    }
...
function addAssemblySwitch(restSwagger, service, options) {
    ...
    for (let pathKey in restSwagger['x-ibm-configuration'].targets[service].paths) {
        ...
        let soapProxyURL = restSwagger['x-ibm-configuration'].
          targets[service]['x-ibm-configuration'].assembly.execute[0].proxy['target-url'];
        let _case = {
            operations: [ {
                verb: 'post',
                path: pathKey
...
* @return opDesc
*/
function createOperationDesc(soapSwagger) {
    let opDesc = {};
    // Rename attribute names, remove xml etc.
    let s = jsonifySwagger(soapSwagger);
    for (let soapPath in s.paths) {
        let opName = soapPath.substring(1);
        // Create a definition for the input and output
        opDesc[opName] = {};
        opDesc[opName].request = createOperationDescInfo(s, opName + 'Input');
        opDesc[opName].response = createOperationDescInfo(s, opName + 'Output');
        opDesc[opName].fault = createOperationDescInfo(s, opName + 'Fault');
    }
    return opDesc;
}
...
* @return info object
*/
function createOperationDescInfo(s, name) {
    ...
    if (soapDef) {
        ...
        let hdrProps = headerProperties(s, soapDef);
        ...
        for (let key in bodyProps) {
            if (info.wrapped) {
                info.def.properties[key] = bodyProps[key];
            } else {
                info.def = bodyProps[key];
                break;
            }
        }
    }
...
*/
function headerProperties(soapSwagger, soapDef) {
    let hdrProperties = {};
    if (soapDef &&
        soapDef.properties &&
        ...
        if (hdr['$ref']) {
            hdr = getRef(soapSwagger, hdr['$ref']);
        }
    ...
    for (let opKey in operationDesc) {
        ...
        if (options.mapSOAPFaults) {
            if (op.fault.def) {
                ...
                let opFault = getDef(restSwagger, op.fault.def);
                if (!restSwagger.definitions.CommonFault) {
                    restSwagger.definitions.CommonFault =
                      jsonifyDefinition(u.deepClone(opFault), options.badgerfishDefs);
                } else {
...
function addRESTDefinitions(soapSwagger, restSwagger, options) {
    ...
    let s = jsonifySwagger(soapSwagger, options.badgerfishDefs);

    for (let ncName in s.definitions) {
        if (!ignore[ncName]) {
            // Get an json version of the xml definition
            restSwagger.definitions[ncName] = s.definitions[ncName];
        }
    }
}
...
*/
function jsonifySwagger(soapSwagger, badgerfishDefs) {
    let s = renameAttributes(soapSwagger, soapSwagger);
    for (let ncName in s.definitions) {
        let def = jsonifyDefinition(s.definitions[ncName], badgerfishDefs);
        s.definitions[ncName] = def;
    }
    return s;
}

function jsonifyDefinition(def, badgerfishDefs) {
    let ignoreKeys = [ 'xml', 'example' ];
    def = u.deepCloneWithIgnoreKeys(def, ignoreKeys);
    if (badgerfishDefs) {
        def = badgerfishDefinition(def);
    }
    return def;
}
...
*/
function badgerfishDefinition(obj) {
    if (obj === null) {
        return null;
    }
    ...
    if (obj instanceof Array) {
        ...
        var len = obj.length;
        for (var i = 0; i < len; i++) {
            cloned = badgerfishDefinition(obj[i]);
            child.push(cloned);
        } // end for
    } else {
        child = {};
        if (isScalar(obj)) {
            ...
            let format = getScalarFormat(obj);
            convertToBadgerfishScalar(child.properties['$'], format);
        } else {
            for (var key in obj) {
                if (key.startsWith('@')) {
                    // Convert schema to badgerfish
                    let format = getScalarFormat(obj[key]);
                    cloned = u.deepClone(obj[key]);
                    convertToBadgerfishScalar(cloned, format);
                } else {
                    cloned = badgerfishDefinition(obj[key]);
                }
                child[key] = cloned;
            }
        } // end for
...
*/
function renameAttributes(swagger, obj) {
    if (obj === null) {
        return null;
    }
    if (typeof obj !== 'object') {
    ...
    if (obj instanceof Array) {
        ...
        var len = obj.length;
        for (var i = 0; i < len; i++) {
            cloned = renameAttributes(swagger, obj[i]);
            child.push(cloned);
        } // end for
    } else {
        child = {};
        for (let key in obj) {
            cloned = renameAttributes(swagger, obj[key]);
            let def = getDef(swagger, obj[key]);
 ...
 */
function addAssemblyWiring(swagger, operationDesc) {
    // Find operation-switch
    ...
    for (let i = 0; i < opSwitch.case.length; i++) {
        ...
        let actions = isWrapped(path, operationDesc, 'request') ?
            getWireActionsWrapped(swagger, inputMap.inputs, inputMap.outputs) :
            getWireActions(swagger, inputMap.inputs, inputMap.outputs);
        inputMap.actions = inputMap.actions.concat(actions);
        actions = isWrapped(path, operationDesc, 'response') ?
            getWireActionsWrapped(swagger, outputMap.inputs, outputMap.outputs) :
            getWireActions(swagger, outputMap.inputs, outputMap.outputs);
...
* @param map.outputs
**/
function getWireActions(swagger, inputs, outputs) {
    let inPath, inDef, outPath, outDef;
    if (inputs.request) {
        inDef = getDef(swagger, inputs.request.schema);
        inPath = 'request';
    } else if (inputs.input) {
        let schema = getDef(swagger, inputs.input.schema);
        let body = schema.properties['Envelope'].properties['Body'];
        let name = getKey(body.properties);
        inDef = getDef(swagger, body.properties[name]);
        inPath = 'input.Envelope.Body.' + name;
    }

    if (outputs.response) {
        outDef = getDef(swagger, outputs.response.schema);
        outPath = 'response';
    } else if (outputs.body) {
        let schema = getDef(swagger, outputs.body.schema);
        let body = schema.properties['Envelope'].properties['Body'];
        let name = getKey(body.properties);
        outDef = getDef(swagger, body.properties[name]);
...
* @param map.outputs
**/
function getWireActionsWrapped(swagger, inputs, outputs) {
    let actions = [];
    if (inputs.request) {
        let inDef = getDef(swagger, inputs.request.schema);
        let schema = getDef(swagger, outputs.body.schema);
        let body = schema.properties['Envelope'].properties['Body'];
        ...
        if (inDef.properties && inDef.properties.header) {
            actions.push(getAction('request.header',
                                   getDef(swagger, inDef.properties.header),
                                  'body.Envelope.Header',
                                   getDef(swagger, header)));
        }
        // Action for body
        ...
        let outDef = getDef(swagger, outputs.response.schema);
        let schema = getDef(swagger, inputs.input.schema);
        let body = schema.properties['Envelope'].properties['Body'];
        let header = schema.properties['Envelope'].properties['Header'];
        ...
        if (outDef.properties && outDef.properties.header) {
            actions.push(getAction('input.Envelope.Header',
                                   getDef(swagger, header),
                                   'response.header',
                                   getDef(swagger, outDef.properties.header)));
        }
        // Action for body
...
function getRef(swagger, ref) {
    ...
    for (let i = 0; i < keys.length; i++) {
        if (keys[i] !== '#') {
            def = def[keys[i]];
            if (!def) {
                throw new Error('The reference ' + ref + '  does not exist');
            }
        }
    }
    return def;
...
*/
function getDef(swagger, obj) {
    if (obj['$ref']) {
        return getRef(swagger, obj['$ref']);
    }
    return obj;
}
