 ...
 * @param {Object} inputFlags - Additional input
 * @return {Object} swagger.definitions
 **/
function getDefinitionsForXSD(filename, auth, rootElementList, inputFlags) {
    // Force arrays
    ...
    // of the schemas
    let def = createTemporaryWSDL(filenameList, authList);
    ...
    def.then(function(wsdlContent) {
        ...
        try {
            parse.getJsonForWSDL(filenameList, authList).then(function(data) {
    ...
    jsonDef.promise.then(function(allWSDLs) {
        try {
            ...
            inputFlags.fromGetDefinitionsForXSD = true;
            inputFlags.rootElementList = rootElementList;
            var swagger = generateSwagger._getSwaggerForService(wsdlEntry, serviceName, wsdlId, inputFlags);
...
function createTemporaryWSDL(filenames, auths) {
    let def = q.defer();
    ...
    getTargetNamespacesForXSD(filenames, auths).then(function(fileEntryList) {
        parseUtils.getFileContent(templateWSDL, templateWSDL, null).then(function(wsdlContent) {
 ...
 */
function getTargetNamespacesForXSD(filenames, auths) {
    var def = q.defer();
    var files = [];
    var promiseList = [];
    ...
    for (var i = 0; i < filenames.length; i++) {
        promiseList.push(_getTargetNamespacesForXSD(filenames[i], auths[i]));
    }
 ...
 */
function _getTargetNamespacesForXSD(filename, auth) {
    var def = q.defer();
    var files = [];
