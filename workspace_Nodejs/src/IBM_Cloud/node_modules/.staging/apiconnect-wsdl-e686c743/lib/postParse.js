...
function checkForWSDLImports(files, auth, options) {
    ...
    try {
        ...
        for (i = 0; i < rootLen; i++) {
            ...
            var impDef = checkOneWSDLImport(rootWSDL, allWSDLs, files, auth, options);
            impDefs.push(impDef);
        } // end for
...
function mergeWSDL(targetJson, sourceFile, targetFileNamespaces) {
    ...
    var wsdlNamespaces = u.deepClone(sourceFile.namespaces);
    for (var key in targetFileNamespaces) {
        if (!wsdlNamespaces[key]) {
            ...
            for (var j = 0; j < len; j++) {
                ...
                if (entry.fullName == location || entry.filename == location) {
                    ...
                    if (entry.type == 'wsdl') {
                        ...
                        impDef = expandWSDLImports(entry.json, relativeFilename, files, mergeList, alreadySeen, refFile, options);
                    } else {
        ...
        if (!foundMatch && refFile.context != 'zip') {
            ...
            if (!alreadySeen[filename]) {
                ...
                var fileDef = parseUtils.getFileContent(location, filename, auth,
                  'location with a value of "' + imp['undefined'].location + '"');
            ...
            let msg = 'Failed to find the following file in the zip archive: ' + location +
            ' referenced in file: ' + refFile.filename +
            '. Ensure that the file is packaged in the zip file.';
            if (!options || !options.validate) {
                msg += ' If using WSRR, this error may be an indication that WSRR does not contain the file or is setup incorrectly.';
            }
            def.reject(new Error(msg));
        } else if (alreadyKnown) {
            def.resolve();
        } else if (!foundMatch) {
            def.reject(new Error('The following file is not found: ' + location +
        '. Ensure that the file is packaged in a zip file.'));
        }
    } else {
        def.resolve();
    }
...
function expandOneSchemaImport(imp, baseFilename, files, mergeList, alreadySeen, parentNS, mustMatchNS, refFile, auth, isWSDLTypesSchema, options) {
    var def = q.defer();
    ...
    if (location) {
            ...
            for (var k = 0; k < len; k++) {
                ...
                if (entry.fullName == location || entry.filename == location) {
                    ...
                    if (entry.type == 'xsd') {
                        ...
                        // recurse into the referenced schema to check for more imports
                        var iDef = expandSchemaImports(entry.json.definitions.types.schema, relativeFilename, files, mergeList, alreadySeen, refFile, false, options);
        ...
        if (!foundMatch && refFile.context != 'zip' && (!options || !options.selfContained)) {
            ...
            if (!alreadySeen[filename]) {
                ...
                files.push(file);
                var fileDef = parseUtils.getFileContent(location, filename, auth,
                  'schemaLocation with a value of "' + imp['undefined'].schemaLocation + '"');
                fileDef.then(function(content) {
                    if (content == location) {
                        file.filename = 'MEMORY';
                        file.fullName = 'MEMORY';
...
function processElementNSChildren(children, jsonSchema) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        if (child.xmlns) {
            u.extendObject(jsonSchema.xmlns, child.xmlns);
        }
        if (child.children) {
...
function getElementNS(jsonSchema, elements) {
    if (elements && jsonSchema) {
        for (var elemName in elements) {
            ...
            if (element.xmlns) {
                u.extendObject(jsonSchema.xmlns, element.xmlns);
            }
            if (element.children) {
...
// add the raw schema-specific namespaces from the raw parsed data back
// into the JSON data
