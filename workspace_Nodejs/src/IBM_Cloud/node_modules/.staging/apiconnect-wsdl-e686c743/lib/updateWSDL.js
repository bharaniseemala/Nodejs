...
function injectServiceEndpointsIntoWSDLorZIP(inContent, serviceEndpoints, serviceName) {
    ...
    try {
        ...
        inContent = parseUtils.toBuffer(inContent);
        let isZip = inContent.toString('utf8', 0, 4).substr(0, 4) === 'PK\u0003\u0004';
        if (isZip) {
            JSZip.loadAsync(inContent).then(function(outZip) {
                setServiceEndpoint(inContent, serviceEndpoints, null, serviceName).then(function(data) {
                    let result;
            ...
            setServiceEndpoint(inContent.toString(), serviceEndpoints, null).then(function(data) {
                let stringContent = data[0].content;
                let result = {
                    outContent: parseUtils.toBuffer(stringContent),
...
*/
function setServiceEndpoint(filename, serviceEndpoints, auth, serviceName) {
    serviceEndpoints = u.makeSureItsAnArray(serviceEndpoints);
    var def = q.defer();
    var fileDef = q.defer();
    ...
    fetchDef.then(function(rawContent) {
        ...
        if (left == 'PK\u0003\u0004') {
                                    ...
                                    readStream.on('end', function() {
                                        ...
                                        if (entry.fileName.indexOf('.wsdl', nameLen - 5) != -1 || entry.fileName.indexOf('.xsd', nameLen - 4) != -1) {
                                            try {
                                                ...
                                                var encoding = parseUtils.determineEncoding(fileContent, entry.fileName);
                                                ...
                                                if (lookForPorts) {
                                                    ...
                                                    if (isModified) {
                                                        ...
                                                        var file = {
                                                            ...
                                                            content: serializer.serializeToString(wsdlDoc)
                                                        };
                                                        data.push(file);
                                                    }
                                                }
                                            } catch (e) {
                                                fileDef.reject(parseUtils.cleanupError(e));
                                            }
                                        }
                                        zipFile.readEntry();
                                    });
            ...
            try {
                var DOMParser = require('xmldom').DOMParser;
                var wsdlDoc = new DOMParser().parseFromString(rawContent, 'text/xml');
                var isModified = false;
