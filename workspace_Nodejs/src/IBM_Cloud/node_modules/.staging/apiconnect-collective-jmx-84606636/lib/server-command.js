...
var SERVER_COMMAND_PATH = '/IBMJMXConnectorREST/mbeans/WebSphere%3Afeature%3' +
...
var SERVER_COMMAND_OPERATIONS = {
  startServer: 'startServer',
  getServerStatus: 'getServerStatus',
  stopServer: 'stopServer',
};

exports.allocateDeployVariables = allocateDeployVariables;
exports.createDataInRegistry = createDataInRegistry;
exports.getServerOperation = getServerOperation;
exports.getServerList = getServerList;
exports.getSetDataInRegistry = getSetDataInRegistry;
exports.getDescendantData = getDescendantData;
exports.releaseDeployVariables = releaseDeployVariables;
exports.SERVER_COMMAND_OPERATIONS = SERVER_COMMAND_OPERATIONS;
 ...
 */
function getServerOperation(options, operation) {
  ...
  assert(SERVER_COMMAND_OPERATIONS[operation],
  ...
  encoder.addString('');
  return new Payload([SERVER_COMMAND_PATH, operation].join('/'),
 ...
 */
function getServerList(options) {
  var encoder = new Encoder();
  var mbeanResource = [
    COLLECTIVE_PATH,
    'hosts',
  ...
  return new Payload([COLLECTIVE_MBEAN_PATH, 'getChildren'].join('/'),
    'POST', encoder.encodeJson());
}
...
function getDescendantData() {
  var mbeanResource = [
    COLLECTIVE_PATH,
    'hosts',
  ];
  return getSubTree({path: mbeanResource.join('/')});
}
 ...
 * the collective repository of the controller.
 */
function getSetDataInRegistry(options, operation) {
  var operations = {
    getData: 'getData',
    setData: 'setData',
    delete: 'delete',
...
function createDataInRegistry(options) {
  var encoder = new Encoder();

  encoder.addString(options.path);
  encoder.addSimpleValue(null, null, jmxTypes.JavaObject);
  return new Payload([COLLECTIVE_MBEAN_PATH, 'create'].join('/'),
    'POST', encoder.encodeJson());
}
 ...
 * @returns {Payload}
 */
function allocateDeployVariables(options) {
  var encoder = new Encoder();
  ...
  return new Payload([COLLECTIVE_MBEAN_PATH, 'allocateDeployVariables']
    .join('/'), 'POST', encoder.encodeJson());
}
 ...
 * @returns {Payload}
 */
function releaseDeployVariables(options) {
  var encoder = new Encoder();
  assert(options, 'You must provide an object with a `host` value!');
  assert(options.host, 'You must specify the hostname of the machine!');
  encoder.addString(options.host);
  ...
  return new Payload([COLLECTIVE_MBEAN_PATH, 'releaseDeployVariables']
    .join('/'), 'POST', encoder.encodeJson());
}
