...
exports.disableScalingPolicy = disableScalingPolicy;
...
exports.undeployCluster = undeployCluster;
exports.undeployMember = undeployMember;

// Internal functions exported for unit testing
exports.parseMembers = parseMembers;
exports.undeployMembers = undeployMembers;
...
function stopCluster(options, cb) {
  async.waterfall([
    getScalingPolicy.bind(null, options),
...
function getScalingPolicy(options, cb) {
  options = parseConfig(options);
  mandatory(options.clusterName, 'missing clusterName');
 ...
 *  ])
 */
function undeployCluster(options, cb) {
  options = parseConfig(options);
  mandatory(options, 'missing options object!');
  mandatory(options.clusterName, 'missing clusterName!');

  // Below function overrides for unit tests
  var _disableScalingPolicy = options.disableScalingPolicy
    || disableScalingPolicy;
  var _listMembers = options.listMembers || listMembers;
  var _undeployMembers = options.undeployMembers || undeployMembers;
  ...
  function checkDeployStatus(deploy, cb) {
    ...
    }, _checkDeployStatus, function(err, results) {
      if (err) {
        // Print error and skip because we needed to generate error for our own
        // purposes and so async.retry will keep trying, but we also need to
        // pass ALL results (even errors) back to APIC, which won't happen if
        // we invoke cb(err), because it will fail immediately;
        debug(err);
      }
    ...
    function _checkDeployStatus(cb, results) {
      ...
      _request(reqOpts, function(err, res, body) {
        ...
        if (!status || status !== 'FINISHED') {
          return cb(new Error(serverName + ': deploy did not finish!'),
            deployInfo);
        }
 ...
 * @param {string} username - Admin username for controller.
 * @param {string} password - Admin password for controller.
 * @param {Object[].<string, string>} members - Array of objects consisting
 * of memberHost, userDir, and serverName.
 *
 */
function undeployMembers(options, members, cb) {
  return async.map(members, undeployMember.bind(null, options),
    function(err, results) {
      if (err) {
        return cb(err);
      }
 ...
 *
 */
function undeployMember(options, member, cb) {
 ...
 *
 */
function disableScalingPolicy(options, cb) {
  // Optional functions for testing purposes
  var _getScalingPolicy = options.getScalingPolicy || getScalingPolicy;
  var _getUserDefinedInstances = options.getUserDefinedInstances ||
    getUserDefinedInstances;
  ...
  function _disableScalingPolicy(instances, cb) {
    ...
    var _uploadPolicy = options.uploadScalingPolicy ||
      fileService.uploadScalingPolicy;
    options.instances = instances;
    options.scalingEnabled = 'false';
