 ...
 * Q2_2_Translation_Returns (Ben Walsh)
 ...
 * Include an assembly overrides CSS (apimesh/apim-ui#1330) (Chris Bygrave)
 ...
 * remove some old deps (#1479) (Eddie A. Monge)
 ...
 * Bigger click target to expand a product (fixes apimesh/apim-ui#1488) (Chris Bygrave)

 * Show error IDs whena vailable (fixes apimesh/apim-ui#1487) (Chris Bygrave)

 * Add support for named rate and burst limits in product editor (apimesh/apim-ui#1478) (Chris Bygrave)

 * bug #1483 fall back to English for untranslated locale (Philip Taunton)

 * disable name generation (Anthony Ettinger)

 * hide leftnav on project select (Anthony Ettinger)

 * Removed stashed change text (Amrit Bhullar)

 * Approval warning; permission check (Amrit Bhullar)

 * In progress changes (Amrit Bhullar)

 * AMP bar controls fix wrapping (Eddie Monge)

 * Fix bug whenc reating new plans (apimesh/apim-ui#1477) (Chris Bygrave)

 * Improve support for operation-level rate limits (including unlimited) (apimesh/apim-ui#1415) Make sure empty rate-limits / burst-limits arrays aren't left behind (Chris Bygrave)

 * Fix paths to policy partials in compressed scope (apimesh/apim-ui#1472) (Chris Bygrave)

 * Support archived -> retired state transition (apimesh/apim-ui#1473) (Chris Bygrave)

 * enh #1287 add support for auth on WSDL URL fetch (Philip Taunton)

 * enh #1287 use node module and support fetch from URL (Philip Taunton)

 * enh #1287 add chameleon schema support and fault message support (Philip Taunton)

 * enh #1287 add URL support, encoding support and various fixes (Philip Taunton)

 * enh #1287 switch to async file handling (Philip Taunton)

 * enh #1287 add split WSDL support, ZIP support and more schema types (Philip Taunton)

 * First pass offline WSDL handling (Philip Taunton)

 * Move apim-ui over to using the externalized apiconnect-explorer module (apimesh/scrum-api-editor#412) (Chris Bygrave)

 * Fix up white logic policies on the palette in built layer (Chris Bygrave)

 * Fix up apiconnect-assembly gulp task (Chris Bygrave)

 * Pull back the policy partials into apim-ui (apimesh/apim-ui#1330) (Chris Bygrave)

 * show full page with pinned left nav (Eddie Monge)

 * show full page with amp panel (Eddie Monge)

 * validation of unique project attribute (Anthony Ettinger)

 * code consistency changes (Eddie Monge)

 * Check for Bluemix token on publish (Cody Walker)

 * Fix issue with explorer and multiple apps (Joseph Tary)

 * bug #1292 only show links with permission (Philip Taunton)

 * bug #1229 apply permissions for vendor extensions (Philip Taunton)

 * bug #1228 handle read-only for apis and products (Philip Taunton)

 * Add title test to add service button (fixes apimesh/apim-ui#1416) (Chris Bygrave)

 * API / product editors should not corrupt line-endings on converting between JSON and YAML (apimesh/apim-ui#1418) (Chris Bygrave)

 * Add support for multiple rate limits and burst limits (apimesh/apim-ui#1415) (Chris Bygrave)

 * complete product publish promises in series (Cody Walker)

 * reset path after openapi creation (Anthony Ettinger)

 * Fix AMP not reflecting project changes (Joseph Tary)

 * Fix logs not showing after starting (Joseph Tary)

 * Fix infinite refresh on model page (Joseph Tary)

 * skip active project for loopback (Anthony Ettinger)

 * add flag to skip project in validator (Anthony Ettinger)

 * reset form on project settings (Anthony Ettinger)

 * Fix onprem publish path (Cody Walker)

 * bug #1421 make sure logout redirects (Philip Taunton)

 * Check for versionOpts on product publish (Cody Walker)

 * bug #1371 clear previous search results (Philip Taunton)

 * fix width on single tree nav (Anthony Ettinger)

 * use anon function (Anthony Ettinger)

 * add refresh project event (Anthony Ettinger)

 * Show current name in publish dialog (Cody Walker)

 * Autofocus with existing directive (Amrit Bhullar)

 * bug #1405 fix property cleanup (Philip Taunton)

 * allow settings to use existing name (Anthony Ettinger)

 * fix overflowing navtree (Anthony Ettinger)

 * Moved custom common directive (Amrit Bhullar)

 * Auto focus input fields (Amrit Bhullar)

 * Performance tweaks for large swaggers (apimesh/apim-ui#1398) (Chris Bygrave)

 * bug #1390 add polyfill for IE 11 startswith (Philip Taunton)

 * bug #1386 add polyfill to support ES5 (Philip Taunton)

 * Add missing clone tooltip string (apimesh/apim-ui#1384) (Chris Bygrave)

 * fix broken login screens (#1383) (Eddie A. Monge)

 * Update CLI dependencies (Krishna Raman)

 * Added datasource enter key (Amrit Bhullar)

 * Creating new model with enter key (Amrit Bhullar)

 * add missing rootScope dependency (#1381) (Eddie A. Monge)

 * Hide vendor extensions in designer temporarily (apimesh/apim-ui#1377) (Chris Bygrave)

 * Fix typo resulting in duplicate 'done' button (apimesh/apim-ui#1372) (Chris Bygrave)

 * apim-ui 5.2.x pulling in apiconnect-assembly 1.2.x (Chris Bygrave)

 * Add angular resource to login jsp (Cody Walker)

 * Fix messages from getting duplicated (#1370) (Eddie A. Monge)

 * Login page targets multiple envs (Cody Walker)

 * Do not expand logs pane when clicking process url (#1369) (Eddie A. Monge)

 * Fix process incorrectly showing as a gw (#1368) (Eddie A. Monge)

 * ui change for versioning (Cody Walker)

 * fix labels and description for testing datasource (#1366) (Eddie A. Monge)

 * Fix amp controls and urls display issues (#1365) (Eddie A. Monge)

 * add localization to project flow (Anthony Ettinger)

 * hide loopback creation flow (Anthony Ettinger)

 * refresh loopback project (Anthony Ettinger)

 * convert button css to less (Anthony Ettinger)

 * Use $state.reload (Anthony Ettinger)

 * fixup from eddie's code review (Anthony Ettinger)

 * default to workspace parent path (Anthony Ettinger)

 * ux fixups for project flows (Anthony Ettinger)

 * hide delete action for active workspace (Anthony Ettinger)

 * hide default openapi template selection (Anthony Ettinger)

 * fixup remove project confirmation modal (Anthony Ettinger)

 * add navigational tree ux for existing project flow (Anthony Ettinger)

 * remove isOpen class from project-nodes (Anthony Ettinger)

 * add project nav to secondary node-tree nav (Anthony Ettinger)

 * add an existing project flow (Anthony Ettinger)

 * use chosen displayName for existing projects (Anthony Ettinger)

 * add openapi project creating flow (Anthony Ettinger)

 * force lowercase for project slugs (Anthony Ettinger)

 * add support for creating an openapi project (Anthony Ettinger)

 * open/close directory arrow (Anthony Ettinger)

 * fix jshint errors (Anthony Ettinger)

 * hide dirs that start with a dot in directory tree (Anthony Ettinger)

 * open up the workspace dir in the directory tree (Anthony Ettinger)

 * add comment for openapi call (Anthony Ettinger)

 * add stub for openapi initialization flow (Anthony Ettinger)

 * add loopback project settings dialog (Anthony Ettinger)

 * remove workspace dir path from leftnav (Anthony Ettinger)

 * match project list in leftnav to design (Anthony Ettinger)

 * clean up UI to match design (Anthony Ettinger)

 * add template selection to loopback creation flow (Anthony Ettinger)

 * add design to file picker node-tree nav (Anthony Ettinger)

 * highlight selected node (Anthony Ettinger)

 * add ability to open/close a node-tree (Anthony Ettinger)

 * add navigable tree list of folders (Anthony Ettinger)

 * add sidebar dropdown menu (Anthony Ettinger)

 * Data source connection change warning (#1361) (Eddie A. Monge)

 * Add AMP design: move run panel to tab at bottom (#1359) (Eddie A. Monge)

 * Teach Catalog Create about portal type IDPs #1331 (Megan Walker)

 * Align jwt policy text to KC (J. Davidse)

 * Cleanup to match auth plugin changes (Krishna Raman)

 * Reformatted parameters and handling (Amrit Bhullar)

 * fix radio buttons and refresh products (Amrit Bhullar)

 * Better error on publish failure (R. Moorhouse)

 * Tweaks to jwt-generate help text (J. Davidse)

 * Disable custom policies for bluemix public (apimesh/apim-ui#1349) (Chris Bygrave)

 * Add license desc to package.json (Eddie A. Monge Jr)

 * Changed name attribute to successfully add new API to existing product (Amrit Bhullar)

 * update the description to make it clearer, or understandable.. Thanks Chris (Shiu F Poon)

 * bug #1215 make sure product is refreshed after publish (Philip Taunton)

 * Updated jwt-validate JSON web token default entry (J. Davidse)

 * bug #1251 fix product name overflow (Philip Taunton)

 * bug #1276 fix property value column widths (Philip Taunton)

 * apimesh/scrum-api-editor#501 fix combo box directive for Angular 1.5 (Philip Taunton)

 * Add beta warning for portal-delegated registries (apimesh/apim-ui#1341) (Chris Bygrave)

 * s/client_id/client-id (Shiu F Poon)

 * Added var to define name to ignore jshint warning (Amrit Bhullar)

 * bugfix: removed protocol from auth endpoint (Cody Walker)

 * JWT Policy changes: Issues 96 and 97 (Belinda Thompson)

 * Remove erroneous one-time-bind from lifecycle product version state field (apimesh/apim-ui#1262) (Chris Bygrave)

 * Service support for ws invoke (apimesh/apim-ui#1333) (Chris Bygrave)

 * Drop in /oauth2/issued to allow user management of gratned permissions (if DP is handling token revocation) (apimesh/apim-ui#1329) (Chris Bygrave)

 * Enable Standalohne micro gateway. Remove broken download links (Megan Walker)
 ...
 * Minimized extra code and optimized for better variable setting (Amrit Bhullar)

 * Added error callback for failed http case when adding to existing product (Amrit Bhullar)

 * Syncing up branch with latest changes from master Merge branch 'master' of github.ibm.com:apimesh/apim-ui into fix/product-from-api (Amrit Bhullar)

 * Fix issue with gulp not regenerating less (Joseph Tary)

 * use apimUser for liberty stack (Peter El-koreh)

 * Removed commented out code and unused watch function (Amrit Bhullar)

 * Syncing up branch for adding multiple products to new api (Amrit Bhullar)

 * change credentials for liberty controller (Peter El-koreh)

 * UI to download microgateway configuration file, disabled by feature flag #1065 (Megan Walker)

 * Text overflow for API names in explorer (fixes apimesh/apim-ui#1320) (Chris Bygrave)

 * Generate content when navigated via the toc, in addition to scroll (apimesh/scrum-api-editor#515) (Chris Bygrave)

 * Support for DataPower as a token revocation endpoint (apimesh/apim-ui#1319) (Chris Bygrave)

 * Fix tests to end correctly instead of hanging (Peter El-koreh)

 * fix explore to work with new Notes loopback app (Peter El-koreh)

 * Adding new API to multiple existing products (Amrit Bhullar)

 * clean up apic-stack-api (Peter El-koreh)

 * Handle collective name conflict (Biniam Admikew)

 * have three options for loopback app (Peter El-koreh)

 * use notes loopback app (Peter El-koreh)

 * Enable portal delegated user registry to show up in UI #1294 (Megan Walker)

 * Add nls strings for portal api port configuration apimesh/cmc#406 (Megan Walker)

 * Populate items porperty for a body parameter of type array (apimesh/apim-ui#335) (Chris Bygrave)

 * add tools temporarily to repo until test tools repo resolved (Peter El-koreh)

 * Remove visible-in-gateway option from UI (apimesh/scrum-edge-gateway#411) (Chris Bygrave)

 * Implemented OAuth popup for Bluemix login (Cody Walker)

 * e2e test for qa team and jenkins (Peter El-koreh)

 * add wait for angular (Peter El-koreh)

 * add a wait for angular after loading product edit (Peter El-koreh)

 * publish to hogan (Peter El-koreh)

 * fix up gulp test (Peter El-koreh)

 * cleanup code (Peter El-koreh)

 * modify datasource tests to use utils (Peter El-koreh)

 * clean up (Peter El-koreh)

 * restructured automation tests (Peter El-koreh)

 * create loopback app as part of gulp setup (Peter El-koreh)

 * set CONFIG_DIR for explorer (Peter El-koreh)

 * use apiconnect-cli-test-support module (Peter El-koreh)

 * Relative positioning on the add operation dropdown trigger (fixes apimesh/apim-ui#1277) (Chris Bygrave)

 * switch-related strings (#1298) (Chris Bygrave)

 * Ensure we handle longin page being returned in transformResponse of CmCIDP.query (Megan Walker)

 * First code drop for standalone micro gateway [disabled via feature toggle] #1065 (Megan Walker)

 * Fix broken button on wsdl import (fixes #1293) (Chris Bygrave)

 * Re-enable polling thread handling for apimesh/cmc-ui#299 (Megan Walker)

 * Fix a jump-to-policy bug (Chris Bygrave)

 * expression building (Chris Bygrave)

 * assembly otherwise string (Chris Bygrave)

 * Support for wsdl-load in map (#1286) (Chris Bygrave)

 * Split common directives in apimUI using the apim.common module needed for apimesh/cmc-ui#369 (Megan Walker)

 * revert modal instance injection into currentUser controller (Anthony Ettinger)

 * fix modal dismissal on ui-bootstrap (Anthony Ettinger)

 * dismiss modal properly (Anthony Ettinger)

 * fix bug with missing lodash function (Anthony Ettinger)

 * stop-on-error strings (apimesh/apim#2654) (Chris Bygrave)

 * Add policy strings (apimesh/apiconnect-assembly#1273) (Chris Bygrave)

 * Disables model Add button when creating new model (Amrit Bhullar)

 * allow duplicate ids to be deleted. (Anthony Ettinger)

 * fix unit tests after angular upgrade (Anthony Ettinger)

 * add proper filename (Anthony Ettinger)

 * rename module (Anthony Ettinger)

 * Browserify highlight.js (#238) (Chris Bygrave)

 * Remove bower_components (#238) (Chris Bygrave)

 * Move all bower deps to npm (#238) (Chris Bygrave)

 * fix jshint errors with unit test karma/chai globals (Anthony Ettinger)

 * add use strict to unit tests (Anthony Ettinger)

 * Prompt for unsaved changes for edited model or data source (Amrit Bhullar)

 * add l10n translations (Anthony Ettinger)

 * add stub for styleguide components (Anthony Ettinger)

 * switch from chrome to phantomjs-2 (Anthony Ettinger)

 * add karma to npm test command (Anthony Ettinger)

 * add unit tests for ProjectsServices (Anthony Ettinger)

 * add unit test for addProject (Anthony Ettinger)

 * update bower libs (Anthony Ettinger)

 * add working unit test for project services (Anthony Ettinger)

 * add sample timeout test and fake promises (Anthony Ettinger)

 * add updated bower packages (Anthony Ettinger)

 * example unit test (Anthony Ettinger)

 * $ref _or_ type / format, not both (apimesh/apim-ui#1244) (Chris Bygrave)

 * Bump package.json so we can start building 5.0.1 (Megan Walker)

 * Continue-on-error strings (apimesh/apim#2398) (Chris Bygrave)

 * Fix for oracle datasources (elkorep)

 * Replace dependency on json-schema-view (#1225) (Chris Bygrave)

 * Jump to source semantics (apimesh/apim-ui#1246) (Chris Bygrave)

 * Add HEAD method string (Chris Bygrave)

 * Add more features for publish (elkorep)

 * Ensure we reset the IDP form to pristine so it can be saved again. apimesh/cmc-ui#396 (Megan Walker)

 * New strings for use in the mapper (#1237) (Chris Bygrave)

 * tls-profile should be empty string apimesh/apim#2352 (Spoon)

 * bug #1198 fix product title filtering (Philip Taunton)

 * bug #1234 fix button spacing and tab order (Philip Taunton)

 * #1231 switch to material icons (Philip Taunton)

 * Fix properties on server models (Joseph Tary)

 * bug #1199 make sure initial navigation highlight is correct (Philip Taunton)

 * Only add the portal scope when public is checked on IDP settings and for CMC (D. Seager)

 * Integrate e2e automation tests (elkorep)

 * Handle pending state in deployment view (apimesh/apim#1619) (Chris Bygrave)

 * Fix confirmation not showing on product and API pages (Joseph Tary)

 * Updating branch for apim cancel buttons (Amrit Bhullar)

 * Provide more useful error message to user at API create time, include leading slash in regex (#1191) (Chris Bygrave)

 * Provide schema download for vendor extension (#1058) (Chris Bygrave)

 * Make sure array copy inserts at next index in json editor (Chris Bygrave)

 * Fix multiple extension bug, expand out toc for extensions (Chris Bygrave)

 * replace where with filter for lodash (Anthony Ettinger)

 * Searchable extensions, tidied up dialogs (Chris Bygrave)

 * Keep hold of the id across YAML->JSON conversions (#686) (Chris Bygrave)

 * OpenAPI Extension -> OpenAPI Extensions (Chris Bygrave)

 * Link to source view direct from vendor extension (Chris Bygrave)

 * Tidy up the vendor extensions management panel (Chris Bygrave)

 * Styling tweaks for vendor extensions within the API editor (Chris Bygrave)

 * Clean up build warning about duplicate variable (Chris Bygrave)

 * Dutch TVT Translation Returns (Ben Walsh)

 * Integrate with externalized assembly css, and mapper-schema (Chris Bygrave)

 * UI support for formFactor property (apimesh/bluemix-ui#353) (Chris Bygrave)

 * enh #367 add name field to developer orgs (Philip Taunton)

 * All catalogs -> Dashboard (fixes #1173) (C. Bygrave)

 * key for datapower domain help field (D. Seager)

 * Fix dodgy behaviour when clearing out the scope name field to type your own replacement (fixes apimesh/apim-ui#1092) (Chris Bygrave)

 * Fix included APIs count on plan editor (fixes apimesh/apim-ui#634) (Chris Bygrave)

 * bug apimesh/scrum-api-editor#425 fix properties navigation (Philip Taunton)

 * bug #1174 prevent property duplication (Philip Taunton)

 * Fix a jshint error in dataSourcesController (#1180) (Chris Bygrave)

 * Disabled all actions under Models when deleting a model (Amrit Bhullar)

 * Improve error message for datasource test (Joseph Tary)

 * String for a property-less policy (#1171) (Chris Bygrave)

 * Clarifying OAuth Provider text (Matthew Pybus)

 * add back the copyright statement, thanks cb #1163 (Spoon)

 * remove the reate, thanks cb :-) (Spoon)

 * fix my snafu for asking the refresh_token to be 5400 (Spoon)

 * bug #867 use same ref name in toc and content (Philip Taunton)

 * Locate pending text properly in deployed product screen (#1150) (Chris Bygrave)

 * bug #1019 make default product template consistent (Philip Taunton)

 * Controller Info title apimesh/cmc-ui#348 (Megan Walker)

 * Update en.json (C. Bygrave)

 * Brackets -> Parentheses (C. Bygrave)

 * Search reflects controllers too apimesh/cmc-ui#270 (Megan Walker)

 * Update Japanese version of webapi.js so load is load rather than burden - v5001 double fix (B. P. Walsh)

 * Help label apimesh/cmc-ui#337 (Megan Walker)

 * Enable product warning / error dropdown (fixes #1161) (Chris Bygrave)

 * Break browserified validate out from usemin layer (#1159) (Chris Bygrave)

 * Add missing word apimesh/cmc-ui#377 (Megan Walker)

 * Positioning Cancel button to left of all editors/dialogs in API Manager (Amrit Bhullar)

 * Fix bad jshintignore entry (Joseph Tary)

 * bug #1141 fix odd properties behaviour (Philip Taunton)

 * Keep LH indicator correct in explorer as navigation occurs (apimesh/scrum-api-editor#418) (Chris Bygrave)

 * Suppress security table when there are none enabled at API level, and extend the explorer RHS to fill alll vertical space without paths (apimesh/scrum-editor#417, apimesh/scrum-editor#419) (Chris Bygrave)

 * Tidy up leftovers from externalized assemblies (Chris Bygrave)

 * bug #1140 make text-only buttons show more clearly (Philip Taunton)

 * bug #1129 fix generated snippet URLs (Philip Taunton)

 * bug #944 prevent username being auto-filled (Philip Taunton)

 * bug #890 fix nested checkbox rendering on IE (Philip Taunton)

 * enh #1054 add creation date ordering to catalogs/apps (Philip Taunton)

 * fix download log files nls. apimesh/cmc-ui#372 (Megan Walker)

 * bug #850 keep left nav in sync after page refresh (Philip Taunton)

 * bug #521 stop version separating from name (Philip Taunton)

 * bug #819 make trash can hitbox a bit bigger (Philip Taunton)

 * Fix apiconnect-validate dependency version (Chris Bygrave)

 * Wire up cmd+s to the save function for various editor screens (#1064) (Chris Bygrave)

 * bug #521 prevent banner buttons wrapping on narrow screens (Philip Taunton)

 * List owner role explicitly in catalog permissions (#955) (Chris Bygrave)

 * Add user email or name to user menu for apimanager and apidesigner (fixes #1013) (Chris Bygrave)

 * Default to not referencing product on deprecate (fixes #1095) (Chris Bygrave)

 * Timestamps in deployed product list (fixes #1099) (Chris Bygrave)

 * bug #1020 remove SCIM check for portal type (Philip Taunton)

 * Clean up display of long cert names (fixes #1121) (Chris Bygrave)

 * Auto-focus name field in new model and data source dialogs (fixes apimesh/scrum-api-editor#447) (Chris Bygrave)

 * bug #500 detect missing catalog ID (Philip Taunton)

 * bug #887 fix select rendering in IE 11 (Philip Taunton)

 * Fix up operation-level security removal logic (#865) (Chris Bygrave)

 * bug #1038 use material checkboxes in product plan editor (Philip Taunton)

 * md-checkbox in developers filter (fixes #1040) (Chris Bygrave)

 * material checkboxes for deployed produt filtering (fixes #1039) (Chris Bygrave)

 * Fix custom policy rendering (apimesh/apim-ui#903) (Chris Bygrave)

 * Clear out type and format when switching definition property to $ref (#1003) (Chris Bygrave)

 * Suppress show-me links in assembly for warnings (#496) (Chris Bygrave)

 * Make sure items gets created proeprly in definitions for array types (#988) (Chris Bygrave)

 * Remove trailing $ref when definition property type changed from definition to simple type (#1023) (Chris Bygrave)

 * Remove one-time bind to definitions (#1025) (Chris Bygrave)

 * Use Sway as an apiconnect-validate service rather than directly (apimesh/apim-ui#1090) (Chris Bygrave)

 * bug #1037 put material checkboxes on API select panel (Philip Taunton)

 * Fix up nav links (fixes apimesh/apm-ui#1101) (Chris Bygrave)

 * Prevent 'Owner' role being assigned to new users (fixes apimesh/apim-ui#1053) (Chris Bygrave)

 * bug #938 make firefox happy with min-height (Philip Taunton)

 * bug #940 fix dev org spacing (Philip Taunton)

 * Remove redundant bower packages (apimesh/apim-ui#1090) (Chris Bygrave)

 * Introduce validation for product (apimesh/apim-ui#1090) (Chris Bygrave)

 * Combine UI & CLI validation logic (apimesh/apim-ui#1090) (Chris Bygrave)

 * bug #975 dont show add app until we have collectives (Philip Taunton)

 * bug #826 make site creation info popup work where it should (Philip Taunton)

 * enh #1084 flip developers and subscriptions (Philip Taunton)

 * bug #977 update terminology (Philip Taunton)

 * ignore apiconnect-assembly.js (Chris Bygrave)

 * bug #806 add validation message for x-ibm-name fields (Philip Taunton)

 * bug #788 update catalog search text (Philip Taunton)

 * bug #1032 fix duplicate notifications on screen (Philip Taunton)

 * bug #1042 prevent button getting cut off at various zoom levels (Philip Taunton)

 * bug #1031 make the inline login clearer (Philip Taunton)

 * bug #1051 use OpenAPI consistently everywhere (Philip Taunton)

 * Line up version number between assembly and apim-ui (Chris Bygrave)

 * Tidy up .gitignore (Chris Bygrave)

 * Wire up apim-ui to use external apiconnect-assembly component (Chris Bygrave)

 * Target connector type-ahead (#1056) (Chris Bygrave)

 * Populate custom errors in error type-ahead (#1055) (Chris Bygrave)

 * Fix error when no schemes are defined (Joseph Tary)

 * Repositioned api editor Schemes above Host and Base Path (Amrit Bhullar)

 * Fix issue where gateway may not be detected (Joseph Tary)

 * Select the default tls profile if the config has nothing selected (D. Seager)

 * Support 'keep' in proxy policy (apimesh/apim#1984) (Chris Bygrave)

 * Revert "example unit test" (Chris Bygrave)

 * Revert "sanitize angular dependencies and bower packages" (Chris Bygrave)

 * Revert "add missing bower libs" (Chris Bygrave)

 * String updates for invoke labels (fixes apimesh/scrum-api-editor#434) (C. Bygrave)

 * Change message detailing the default verb for an invoke (apimesh/scrum-edge-gateway#359) (C. Bygrave)

 * add missing bower libs (Anthony Ettinger)

 * sanitize angular dependencies and bower packages (Anthony Ettinger)

 * Update en.json (Gary Chapman)

 * Fill Name field only when text entered into title (D. Seager)

 * Revert "Feature/e2e integration bluemix publish" (Joseph Tary)

 * fix conflicts (elkorep)

 * add run code (elkorep)

 * explore tests (elkorep)

 * fix account (elkorep)

 * add bluemix publish automation (Jisoo Lee)

 * refactor tests to match apimui conventions (Karan Randhawa)

 * use local app rather than apic loopback gen (Karan Randhawa)

 * add static test app (Karan Randhawa)

 * generate and use test app (Karan Randhawa)

 * Update model-spec.js (Peter El-Koreh)

 * Update api-spec.js (Peter El-Koreh)

 * Update run-spec.js (Peter El-Koreh)

 * fix paths (elkorep)

 * renamed files to fit in with new structure (elkorep)

 * add our tests into apim-ui (elkorep)

 * remove .call (Anthony Ettinger)

 * add logout routine (Anthony Ettinger)

 * Validate Name against the sluggify regexp in API and Product designer (D. Seager)

 * enh #1076 make access tokens always visible (Philip Taunton)

 * CMC name slugs for common code and messages (D. Seager)

 * enh #1076 add ttl fields for OAuth (Philip Taunton)

 * bug #1074 fix regex check order (Philip Taunton)

 * bug #1003 filter out API only IDPs from list (Philip Taunton)

 * Remediate $item, use foreach (Chris Bygrave)

 * add wait for landing page method (Anthony Ettinger)

 * fix project root path (Anthony Ettinger)

 * add protractor binary path for windows (Anthony Ettinger)

 * remove unused ref to Readable stream (Anthony Ettinger)

 * add initial bluemix login test (Anthony Ettinger)

 * Auto test datasource when saving (Joseph Tary)

 * Fix on-blur causing issues with test connection (Joseph Tary)

 * Preliminary support for testing datasource connection (Joseph Tary)

 * Common sluggifier, sluggify API, Product, App, Catalog names (D. Seager)

 * Cloak login page until ready, use absolute script paths for bluemix login purposes (fixes #1043) (Chris Bygrave)

 * bug #1027 fixup popup help page doc links (Philip Taunton)

 * enh #399 add import swagger to offline experience (Philip Taunton)

 * Support multiple security requirements in explorer (#1000) (Chris Bygrave)

 * suppress errors from the list poll thread (fixes apimesh/scrum-api-editor#423) (Chris Bygrave)

 * Support for both AND and OR is security settings in API editor (#1000) (Chris Bygrave)

 * Default array items type to string in parameters (#335) (Chris Bygrave)

 * Make assembly test tool play nice with schemes property (apimesh/scrum-api-editor#382) (Chris Bygrave)

 * No phase given == realized (apimesh/scrum-api-editor#388) (Chris Bygrave)

 * Default gateway to micro offline, dp online (#1014) (Chris Bygrave)

 * Update README (Joseph Tary)

 * bug apimesh/scrum-api-editor#385 fill in type column in offline case (Philip Taunton)

 * bug apimesh/scrum-api-editor#400 change swagger import to OpenAPI (Philip Taunton)

 * Fixed incorrect var name in bluemix-handler (Cody Walker)

 * Translation Returns - Add Dutch (Ben Walsh)

 * Prevent use of non-HTTPS schemes being flagged as an error when targetting MG instead of DP (apimesh/scrum-platform-foundation#570) (Chris Bygrave)


2016-03-21, Version 5.0.232
===========================



2016-03-20, Version 5.0.0
=========================
