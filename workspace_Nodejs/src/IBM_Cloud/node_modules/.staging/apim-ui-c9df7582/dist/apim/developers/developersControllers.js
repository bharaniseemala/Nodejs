...
developers.controller('AddOrgController', ['$scope', '$modalInstance', '$q', 'Role', 'Organization', 'ApimUser', 'UserContext', 'UserRegistry', 'environment', 'orgId', 'idp', addOrgController]);

function addOrgController($scope, $modalInstance, $q, Role, Organization, User, UserContext, UserRegistry, environment, orgId, idp) {
...
function sendMessageController($scope, $modalInstance, DeveloperMessage, userList, orgId) {
  ...
  $scope.send = function() {
    ...
    for (var i = 0; i < len; i++) {
      if (localData.recipients.indexOf($scope.userList[i].email) < 0) {
        localData.recipients.push($scope.userList[i].email);
      }
    } // end for
