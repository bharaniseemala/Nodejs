/********************************************************* {COPYRIGHT-TOP} ***
* Licensed Materials - Property of IBM
* 5725-Z22, 5725-Z63, 5725-U33, 5725-Z63
*
* (C) Copyright IBM Corporation 2017
*
* All Rights Reserved.
* US Government Users Restricted Rights - Use, duplication or disclosure
* restricted by GSA ADP Schedule Contract with IBM Corp.
********************************************************** {COPYRIGHT-END} **/
.apim .listingColumn,
.apim .filterColumn {
  display: inline-block;
  padding: 0 60px 20px;
}
.apim .listingColumn {
  width: calc(100% - 160px);
}
.apim .listingColumn.catalogsEnvironments {
  padding: 0 50px 20px;
}
.apim .listingColumn.centerColumn.stretch {
  margin-top: 0.6rem;
}
.apim .listingColumn.account-settings {
  margin-top: 11rem;
}
.apim .listingColumn ul .primaryBorder {
  border-color: #e0e0e0;
}
.apim .makeSpaceForNav .listingColumn.tabularData {
  width: calc(100% - 480px);
}
.apim .makeSpaceForNav .expanded .listingColumn.tabularData {
  width: calc(100% - 320px);
}
.apim .makeSpaceForNav .expanded .listingColumn.tabularData.catalogsEnvironments,
.apim .makeSpaceForNav .listingColumn.tabularData.catalogsEnvironments,
.apim .makeSpaceForNav .catalogs.centerColumn {
  width: 100%;
}
.apim .makeSpaceForNav .environmentDetails .centerColumn {
  padding: 0;
}
.apim .expanded .listingColumn {
  width: 100%;
}
.apim .expanded .filterColumn {
  display: none;
}
.apim .filterColumn {
  border-left-style: solid;
  border-left-width: 1px;
  min-height: 460px;
  vertical-align: top;
  white-space: normal;
  width: 160px;
}
.apim .centerColumn {
  padding: 25px 60px 0;
}

/*# sourceMappingURL=data:application/json;charset=utf8;base64, */
