...
********************************************************** {COPYRIGHT-END} **/
-->
<div data-ng-controller="CommunityDeveloperOrganizationsController" class="bx--global-light-ui">
    ...
    <div class="bx--responsive-table-container">
      <table class="bx--responsive-table">
        <thead class="bx--table-head">
          <tr class="bx--table-row">
            ...
            <th class="bx--table-header bx--table-sort bxTablePaddingLeft" ng-class="{'bx--table-sort--ascending' : orderDirection, 'bx--table-sort-active' : orderProp == 'displayName'}" data-event="sort" data-ng-click="orderProp = 'displayName'; orderDirection = !orderDirection;">
                <bx-checkbox aria-label="{{developerOrg.displayName}}" data-ng-model="selectAllOrgs" data-ng-model-options="{getterSetter:true, debounce:100}">
                </bx-checkbox>
            </th>
        ...
        <tbody class="bx--table-body">
          ...
          <tr class="bx--table-row bx--parent-row" data-ng-show="developerOrgs && developerOrgs.length === 0">
            <td colspan="5" class="table-empty">{{(spaceId ? 'community-no-developer-orgs-space' : 'community-no-developer-orgs') | translate}}</td>
          </tr>

          <tr class="bx--table-row bx--parent-row" data-ng-show="developerOrgs && developerOrgs.length > 0 && filteredDeveloperOrgs.length === 0">
            <td colspan="5">{{::'community-no-matching-developer-orgs' | translate}}</td>
          </tr>

          <tr class="bx--table-row bx--parent-row" ng-class-even="'bx--parent-row--even'" data-ng-repeat="developerOrg in filteredDeveloperOrgs = (developerOrgs | communityDeveloperOrgsQuery: query | orderBy: orderProp: orderDirection)" data-ng-controller="CommunityDeveloperOrganizationController">
