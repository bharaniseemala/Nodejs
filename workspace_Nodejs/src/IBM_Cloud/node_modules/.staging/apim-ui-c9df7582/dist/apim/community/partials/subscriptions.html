...
********************************************************** {COPYRIGHT-END} **/
-->
<div data-ng-controller="CommunitySubscriptionsController" class="bx--global-light-ui">
    ...
    <div class="bx--responsive-table-container">
      <table class="bx--responsive-table">
        ...
        <tbody class="bx--table-body">
          ...
          <tr class="bx--table-row bx--parent-row bxTablePaddingLeft" data-ng-show="subscriptions && subscriptions.length === 0">
            <td colspan="5" class="table-empty">{{(spaceId ? 'community-no-subscriptions-space' : 'community-no-subscriptions') | translate}}</td>
          </tr>

          <tr class="bx--table-row bx--parent-row bxTablePaddingLeft" data-ng-show="subscriptions && subscriptions.length > 0 && filteredSubscriptions.length === 0">
            <td colspan="5">{{::'community-no-matching-subscriptions' | translate}}</td>
          </tr>

          <tr class="bx--table-row bx--parent-row" ng-class-even="'bx--parent-row--even'" data-ng-repeat="subscription in filteredSubscriptions = (subscriptions | communitySubscriptionsQuery: query | orderBy: orderProp: orderDirection)" data-ng-controller="CommunitySubscriptionController">
