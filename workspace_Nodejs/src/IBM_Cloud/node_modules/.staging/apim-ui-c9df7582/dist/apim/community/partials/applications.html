...
********************************************************** {COPYRIGHT-END} **/
-->
<div data-ng-controller="CommunityApplicationsController" class="bx--global-light-ui">
    ...
    <div class="bx--responsive-table-container">
      <table class="bx--responsive-table">
        ...
        <tbody class="bx--table-body">
          ...
          <tr class="bx--table-row bx--parent-row bxTablePaddingLeft" data-ng-show="consumerApps && consumerApps.length === 0">
            <td colspan="5" class="table-empty">{{(spaceId ? 'community-no-applications-space' : 'community-no-applications') | translate}}</td>
          </tr>

          <tr class="bx--table-row bx--parent-row bxTablePaddingLeft" data-ng-show="consumerApps && consumerApps.length > 0 && filteredApplications.length === 0">
            <td colspan="5">{{::'community-no-matching-applications' | translate}}</td>
          </tr>

          <tr class="bx--table-row bx--parent-row" ng-class-even="'bx--parent-row--even'"
          data-ng-repeat="application in filteredApplications = (consumerApps | communityApplicationsQuery: query | orderBy: orderProp: orderDirection)" data-ng-controller="CommunityApplicationController">
