/********************************************************* {COPYRIGHT-TOP} ***
* Licensed Materials - Property of IBM
* 5725-Z22, 5725-Z63, 5725-U33, 5725-Z63
*
* (C) Copyright IBM Corporation 2017
*
* All Rights Reserved.
* US Government Users Restricted Rights - Use, duplication or disclosure
* restricted by GSA ADP Schedule Contract with IBM Corp.
********************************************************** {COPYRIGHT-END} **/
.model-property-table tbody tr:hover {
  border: 0;
  -webkit-box-shadow: none;
  box-shadow: none;
}
.model-property-table tbody tr:hover td {
  -webkit-box-shadow: none;
  box-shadow: none;
}
.model-property-table tr.deleted td {
  opacity: 0.5;
  -ms-filter: "progid:DXImageTransform.Microsoft.Alpha(Opacity=50)";
  filter: alpha(opacity=50);
}
.model-property-table tr.deleted td:before {
  position: absolute;
  top: 2rem;
  left: 0;
  border-bottom: 1px solid #111;
  content: '';
  width: 100%;
}
.model-property-table .md-input {
  width: 100%;
}
.model-property-table > tbody > tr {
  border: 0;
}
.model-property-table > tbody td {
  line-height: 12px;
  margin: 0;
  position: relative;
}
.model-property-table .center {
  text-align: center;
}
.model-property-table .md-errors-spacer {
  display: none;
}
.model-property-table md-input-container {
  margin: 0;
  padding: 0;
  width: 100%;
}
.model-property-table md-select {
  margin: 0;
  padding: 0;
}
.model-property-table md-checkbox {
  margin: 0 0 0 10px;
  padding: 0;
}
.modal-dialog .modal-content p {
  line-height: 1.75;
  padding-bottom: 1em;
}

/*# sourceMappingURL=data:application/json;charset=utf8;base64, */
